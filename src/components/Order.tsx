...
export default function Order({ order }: OrderProps) {
  const paymentStatusMap: Record<orders.PaymentStatus, string> = {
    ...
    [orders.PaymentStatus.PENDING_MERCHANT]: "Pending merchant approval",
  ...
  return (
    <div className="w-full space-y-5 border p-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Link
          href={`mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent(`Order #${order.number} help`)}&body=${encodeURIComponent(`I need with order #${order.number}\n\n<Describe your problem>`)}`}
